<nav class="fixed top-0 z-50 w-full  bg-gray-900 border-gray-700">
    <div class="px-3 py-3 lg:px-5 lg:pl-3">
        <div class="flex items-center justify-between">
            <div class="flex items-center justify-between md:justify-start rtl:justify-end">
                ...
                <a href="{{ route('dashboard') }}" class="flex ms-2 md:me-24 gap-2">
                    <img src="{{ asset('storage/image/logo.png') }}" class="md:h-8 h-7 rounded-full"
                        alt="Kabaena Logo" />
                    <span
...
<aside id="logo-sidebar"
    class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full  border-r  sm:translate-x-0 bg-gray-900 border-gray-700"
    aria-label="Sidebar">
    <div class="h-full px-3 pb-4 overflow-y-auto  bg-gray-900">
        <ul class="space-y-2 font-medium">
            @if (auth()->user()->role == 'admin' || auth()->user()->role == 'peserta')
                ...
                <li>
                    <a href="{{ auth()->user()->role === 'peserta' ? route('tim.view', auth()->user()->tim_id) : url('/admin/tim') }}"
                ...
                <li>
                    <a href="{{ auth()->user()->role === 'peserta' ? route('schedule.admin.view', auth()->user()->tim_id) : url('/admin/schedule') }}"
                        class="flex items-center p-2  rounded-lg text-white  hover:bg-red-600 {{ Request::routeIs('schedule.*') ? 'bg-red-600' : '' }}">
            ...
            @if (auth()->user()->role == 'admin')
                ...
                <li>
                    <a href="{{ route('highlight.index') }}"
                        class="flex items-center p-2  rounded-lg text-white  hover:bg-red-600 {{ Request::routeIs('highlight.*') ? 'bg-red-600' : '' }}">
                ...
                <li>
                    <a href="{{ route('user.index') }}"
                        class="flex items-center p-2  rounded-lg text-white  hover:bg-red-600 group {{ Request::routeIs('user.*') ? 'bg-red-600' : '' }}">
                        <x-uiw-usergroup-add class="w-[20px]" />
        ...
        <footer class=" rounded-lg shadow mt-4 bg-gray-800 bottom-0">
            <div class="w-full mx-auto max-w-screen-xl p-4 flex items-center justify-center ">
                <span class="text-sm  sm:text-center text-gray-400">© 2024-{{ date('Y') }} Kabaena Cup™. All Rights
                    Reserved.
                </span>
            </div>
        </footer>
